...
public:
    ui() {
        ...
        perflayout->addWidget(gpuGroup, 1, 1);
        ...
        procTable->setHorizontalHeaderLabels({"PID", "USER", "NI", "CPU%", "MEM%", "VIRT", "RES", "COMMAND"});
    
        procTable->horizontalHeader()->setSectionsClickable(true);
        procTable->horizontalHeader()->setSortIndicator(currentSortColumn,currentSortOrder);
        procTable->horizontalHeader()->setSortIndicatorShown(true);
        connect(procTable->horizontalHeader(), &QHeaderView::sectionClicked, this,&SimpleUI::onHeaderClicked);
        
        procTable->horizontalHeader()->setSectionResizeMode(QHeaderView::Stretch);
        procTable->horizontalHeader()->setSectionResizeMode(7, QHeaderView::ResizeToContents);
        ...
        connect(niceBtn, &QPushButton::clicked, this, &SimpleUI::onNiceClicked);
        connect(killBtn, &QPushButton::clicked, this, &SimpleUI::onKillClicked);
        tabWidget->addTab(procTab, "Processes");

        cpuDummy = cpuMon.getRawStats();
        procMon.fetch();
        auto *timer = new QTimer(this);
        connect(timer, &QTimer::timeout, this, &SimpleUI::updateStats);
        timer->start(1000);
        updateStats();
    }
    ...
    void updatestats(){
        ...
        cpuDetailsLabel->setText(QString("Model: %1\n"
                                    ...
                                    .arg(cpu.baseSpeed, 0, 'f', 0)
                                    .arg(QString::fromStdString(cpu.l3Cache))
                                    .arg(formatUptime(cpu.uptimeSeconds))
                                    .arg(cpu.processes));
        ...
        if (gpu.hasGpu) {
            ...
            gpuDetailsLabel->setText(QString("Model: %1\n"
                                            ...
                                            .arg(gpu.tempC, 0, 'f', 0)
                                            .arg(gpu.vramUsedGb, 0, 'f', 2)
                                            .arg(gpu.vramTotalGb, 0, 'f', 2));
        } else {
        ...
        for (const auto &iface : netIfaces) {
            totalRx += iface.rxSpeedKBs;
            totalTx += iface.txSpeedKBs;
        }
        netLabel->setText(
        QString("Total Speed: Down: %1 | Up: %2")
                .arg(QString::fromStdString(networkMonitor::formatSpeed(totalRx)))
                .arg(QString::fromStdString(networkMonitor::formatSpeed(totalTx))));
            ...
            netInfo +=
                QString(
                    ...
                    .arg(QString::fromStdString(
                        networkMonitor::formatSpeed(iface.rxSpeedKBs)))
                    .arg(QString::fromStdString(
                        networkMonitor::formatSpeed(iface.txSpeedKBs)))
                    .arg(iface.totalRxGb)
                    .arg(iface.totalTxGb)
                    .arg(QString::fromStdString(iface.ipv4Address))
                    .arg(QString::fromStdString(iface.ipv6Address));
        }
    ...
    for (const auto &p : procs) {
        ...
        procTable->setItem(row, 1,new QTableWidgetItem(QString::fromStdString(p.user)));
        procTable->setItem(row, 2, new QTableWidgetItem(QString::number(p.nice)));
        procTable->setItem(row, 3,new QTableWidgetItem(QString("%1%").arg(p.cpuPercent, 0, 'f', 1)));
        procTable->setItem(row, 4,new QTableWidgetItem(QString("%1%").arg(p.memPercent, 0, 'f', 1)));
        procTable->setItem(row, 5,new QTableWidgetItem(QString::fromStdString(formatMemory(p.virtMb))));
        procTable->setItem(row, 6,new QTableWidgetItem(QString::fromStdString(formatMemory(p.resMb))));

        string cmd = p.command;
        if (cmd.length() > 65)
            cmd = cmd.substr(0, 62) + "...";
        procTable->setItem(row, 7,new QTableWidgetItem(QString::fromStdString(cmd)));
...
void onKillClicked() {
    ...
    if (row < 0) {
        QMessageBox::warning(this,"Please select a process from the list first.");
        return;
    }
    ...
    if (btn == QMessageBox::Yes) {
        if (killProcess(pid)) {
            ...
            QMessageBox::critical(this, "Failed","You might not have sufficient permissions.");
        }
    }
}
...
void onNiceClicked() {
    int row = procTable->currentRow();
    if (row < 0) {
        QMessageBox::warning(this, "Warning","Please select a process from the list first.");
        return;
    }
    int pid = procTable->item(row, 0)->text().toInt();
    ...
    int newNice = QInputDialog::getInt(
        ...
        QString("Enter new Nice value for PID %1 (-20 to 19)","Lower values is higher priority.")
            .arg(pid),
    ...
    if (ok) {
        if (setProcessPriority(pid, newNice)) {
            ...
            this, "Failed","Setting negative nice values requires admin.");
        }
    }
}
...
private:
    ...
    diskData disk;
    gpuData gpu;
    std::vector<interfaceData> netIfaces;
